...
from .scoring import compute_score, has_clash, excl_arrays
...
def solve_target(name, data):
    ...
    screen_results = _phase1_screen(mol_h, cids, feat_atoms, sites, ec, er, rng)
    ...
    best_score, best_cid, best_params = _phase2_deep(
        mol_h, cids, screen_results, feat_atoms, sites, ec, er, rng,
    )
    ...
    if best_params is not None:
        ...
        best_score, best_params = _local_refine(
            mol_h, best_cid, best_params, best_score, feat_atoms, sites, ec, er,
        )
    ...
    # ── Build output molecule ──
    return _build_output(mol_h, best_cid, best_params, name, best_score), best_score


# ── internal helpers ─────────────────────────────────────────────────────────

def _phase1_screen(mol_h, cids, feat_atoms, sites, ec, er, rng):
    """Quick screen every conformer.  Returns sorted list of (score, cid, params)."""
    results = []
    n_clashing = 0
    for idx, cid in enumerate(cids):
        ...
        guesses.extend(anchor_starts(coords, feat_atoms, sites, rng, n=6))
        ...
        local_best_p = None
        for g in guesses:
            sc, px, clash = optimise_guess(coords, g, feat_atoms, sites, ec, er)
...
def _phase2_deep(mol_h, cids, screen_results, feat_atoms, sites, ec, er, rng):
    """Deep optimise top-K conformers."""
    TOP_K = min(20, len(screen_results)) if screen_results else min(20, len(cids))
    top_cids = (
    ...
    for ci, cid in enumerate(top_cids):
        ...
        # Build diverse starting guesses
        guesses = [np.concatenate([[0, 0, 0], t0])]
        guesses.extend(weighted_svd_alignment(coords, feat_atoms, sites))
        guesses.extend(icp_svd_starts(coords, feat_atoms, sites, n_iters=4))
        guesses.extend(anchor_starts(coords, feat_atoms, sites, rng, n=20))
        guesses.extend(three_point_starts(coords, feat_atoms, sites, rng, n=20))
        for _ in range(20):
            rv = rng.randn(3) * np.pi
            guesses.append(np.concatenate([rv, t0 + rng.randn(3) * 3.0]))
        for rx in [0, np.pi / 2, np.pi, -np.pi / 2]:
            for ry in [0, np.pi / 2, -np.pi / 2]:
                for rz in [0, np.pi / 2]:
                    guesses.append(np.concatenate([[rx, ry, rz], t0]))

        log.debug("    conf %d/%d: %d guesses (Nelder-Mead)...",
                  ci + 1, len(top_cids), len(guesses))

        # Nelder-Mead on every guess
        conf_best = -np.inf
        for g in guesses:
            sc, px, clash = optimise_guess(coords, g, feat_atoms, sites, ec, er)
        ...
        if icp:
            sc, px, clash = score_only_then_refine(
                coords, icp[0], feat_atoms, sites, ec, er,
            )
            if not clash and sc > best_score:
                best_score, best_cid, best_params = sc, cid, px.copy()
            if not clash and sc > conf_best:
        ...
        try:
            ...
            if not has_clash(tc, ec, er) and sc > best_score:
                best_score, best_cid, best_params = sc, cid, res.x.copy()
            if not has_clash(tc, ec, er) and sc > conf_best:
                conf_best = sc
            log.debug("    conf %d DE: score=%.4f", ci + 1, sc)
        except Exception:
            log.debug("    conf %d DE: failed", ci + 1)

        # Basin-hopping on top-5 conformers
        if ci < 5:
            ...
            if bh_start is not None:
                try:
                    ...
                    tc = apply_transform(coords, bh_res.x)
                    sc = compute_score(tc, feat_atoms, sites)
                    if not has_clash(tc, ec, er) and sc > best_score:
...
def _local_refine(mol_h, best_cid, best_params, best_score, feat_atoms, sites, ec, er):
    """Polish the best pose with multiple local optimisers."""
    coords = get_coords(mol_h, best_cid)
    for method in ("Powell", "Nelder-Mead", "COBYLA"):
        res = minimize(
            objective_full,
            best_params,
            args=(coords, feat_atoms, sites, ec, er),
...
def _fallback(mol_h, cids, feat_atoms, sites):
